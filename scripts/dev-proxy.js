...
const server = http.createServer((req, res) => {
  ...
  // Health check endpoint - test if proxy is running
  ...
  // req.url will be like "/api/auth/signIn", we need to forward to API_BASE_URL + req.url
  ...
  const options = {
    ...
    headers: {
      ...
      'User-Agent': req.headers['user-agent'] || 'Tenga-Proxy/1.0',
    },
  };
  ...
  const proxyReq = targetProtocol.request(options, (proxyRes) => {
    // Ensure CORS headers are set FIRST (before copying other headers)
    res.setHeader('Access-Control-Allow-Origin', '*');
    res.setHeader('Access-Control-Allow-Methods', 'GET, POST, PUT, DELETE, OPTIONS, PATCH');
    res.setHeader('Access-Control-Allow-Headers', 'Content-Type, Authorization, X-Requested-With');
